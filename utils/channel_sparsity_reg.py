...
from settings.train_settings import CONV_SPARSITY_PENALTY, LATENT_SPARSITY_PENALTY, LINEAR_SPARSITY_PENALTY, TOPOLOGY


def update_bn(model, conv_penalty=CONV_SPARSITY_PENALTY, linear_penalty=LINEAR_SPARSITY_PENALTY,
              latent_penalty=LATENT_SPARSITY_PENALTY):
    if max(conv_penalty, linear_penalty, latent_penalty) > 0:
        for m in model.modules():
            if isinstance(m, _BatchNorm):
                if isinstance(m, BatchNorm1d):
                    if m.weight.data.shape[0] == TOPOLOGY[0]:
