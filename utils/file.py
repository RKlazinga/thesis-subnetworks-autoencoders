...
def get_topology_of_run(run_id):
    change_working_dir()
    checkpoint_file = [x for x in os.listdir(f"{RUN_FOLDER}/{run_id}") if x.startswith("starting_params-")][0]
...
def get_params_of_run(run_id, epoch=None, device=None):
    ...
    checkpoint_file = f"{RUN_FOLDER}/{run_id}/" + checkpoint_file
    return torch.load(checkpoint_file, map_location=device)


def get_epochs_of_run(run_id):
    change_working_dir()
    files = [x for x in os.listdir(f"{RUN_FOLDER}/{run_id}") if "trained" in x]
